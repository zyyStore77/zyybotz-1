...
*[ • Z Y Y • B O T • ]*`
	    ...
	    reply(`*Terimakasih Telah Terdaftar Di [ • Z Y Y • B O T • ]*`)
	}, 2000)
	}
if (budy.toLowerCase() === `5679`){
	if (isRegistered) return reply('Akun kamu sudah terverfikasi')
...
*Z Y Y B O T*`
        ...
        // console.log(e)
            setTimeout( () => {
	    ikyy.updatePresence(from, Presence.composing)
	    reply(`*Terimakasih Telah Terdaftar Di ZyyBOT*`)
	}, 2000)
	}
///// Auto Read
...
`
    ...
    ikyy.sendMessage(from, { contentText: `${menu}`, footerText: 'Created by ©Fathir', buttons: [{ buttonId: `!command`, buttonText: { displayText: 'LIST MESSAGE' }, type: 1 },{ buttonId: `!sewabot`, buttonText: { displayText: '🏷️ SEWA BOT' }, type: 1 },{ buttonId: `!owner`, buttonText: { displayText: '👥 OWNER' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: ikyads, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
              break
case 'f':
case 'lock':
 ikymekyy = {
...
┗━▹ `
sendButMessage(from, wibu, `created Zyy botz`, [
          {
            buttonId: `!menu`,
            buttonText: {
...
`
   sendButMessage(from, wibu, `created Zyy botz`, [
        ...
        case 'simi':
              ...
              buttonsMessage = {footerText:'crated by Zyy botz', imageMessage: imageMsg,
...
`
   sendButMessage(from, wibu, `created Zyy botz`, [
        ...
        case 'wasted':
		      ...
		      buttons = [{buttonId: `!infoig`,buttonText:{displayText: ` Follow @zyystore_`},type:1}]
              imageMsg = (await ikyy.prepareMessageMedia(buff, "imageMessage", { thumbnail: buff, })).imageMessage
              buttonsMessage = {footerText:'crated by Zyy botz', imageMessage: imageMsg,
              contentText:`Follow @zyystore_`,buttons,headerType:4}
              prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: freply})
              ikyy.relayWAMessage(prep)
break
				case 'quotesmaker':
      ...
      case 'broadcast2':
             ...
             for (let _ of anu) {
             ikyy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 BROADCAST [ • Zyy BOT • ] 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
ikyy.sendMessage(_.jid, 
	    {"contentText": `*「 BROADCAST [ • ZYY BOT • ] 」*\n\n${body.slice(4)}`,
	    "footerText": '©Created by FathirAnwarudin',
	    "buttons": [
	    {"buttonId": `!owner`,
      ...
      case 'broadcast4':
             ...
             bc = await ikyy.downloadMediaMessage(encmedia)
             for (let _ of anu) {
             ikyy.sendMessage(_.jid, bc, image, {quoted:freply,caption: `*「 BROADCAST [ • ZY BOT • ] 」*\n\n${body.slice(4)}`})
}
             reply('Suksess broadcast')
             } else {
             for (let _ of anu) {
...
case 'bc3':
             ...
             for (let _ of anu) {
             ikyy.sendMessage(_.jid, { contentText: `*🌹 SIARAN ZYYBOT 🌹*\n\n${body.slice(4)}`, footerText: '©Created by FathirAnwarudin', buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'MENU' }, type: 1 }, { buttonId: `${prefix}owner`, buttonText: { displayText: 'OWNER' }, type: 1 }], headerType: 6, locationMessage: { degreesLatitude: 0, degreesLongitude: 0, jpegThumbnail: bc2 }}, 'buttonsMessage')
}
             reply('Suksess broadcast')
}
             break
...
`
 ...
 ♥️ Zyy botz`, imageMessage: imageMsg,
               buttons: buttons,
               headerType: 1
}
...
ikyy.sendMessage(sender, "Kode Verify : 5679 \n  \nAnda juga dapat mengetuk tautan ini untuk verifikasi: https://wa.me/62882141905083?text=5679 \n  \nKode ini bersifat sementara dan bisa jadi sama dengan orang lain\nSupport Gua Terus🥰 https://youtube.com/channel/UCdWYt4sTS5IA-BJszuYVqWQ", text, {quoted: freply})
break
case 'sfire':
{
...
`
  buttons = [{buttonId: `${prefix}bukti`,buttonText:{displayText: `KIRIM BUKTI TF`},type:1},{buttonId: `${prefix}payment`,buttonText:{displayText: `PAYMENT LAIN`},type:1}]
      ...
      break
         case 'dana':
              gopeynya = `https://i.ibb.co/xLLzhvc/IMG-20210928-205639.jpg`
            buff = await getBuffer(gopeynya)
            teksnya = `
Hai kak ${pushname}
...
//------------------< Ingfo Bot >-------------------
      case 'runtime':
       ...
       case 'sc': 
       case 'src':
              textImg(`Bot ini menggunakan sc Private Lah Mana Mungkin Gua Kasih:v`)
              break
      case 'ping':
      case 'speed':
              timestampe = speed();
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Jangan Lupa Follow Ig Owner Ya : https://bit.ly/igzyy`)
             break
      case 'ikyygroup':
